 ...
 * 
 * Мост между GyroReceiver (ориентация телефона) и Drone_Inputs (управление дроном).
 * 
 * Логика:
 *   - Наклон телефона вперёд/назад (pitch) → pitch дрона (cyclic.y)
 *   - Наклон телефона влево/вправо (roll) → roll дрона (cyclic.x) 
 *   - Поворот телефона по горизонтали (yaw) → yaw дрона (pedals)
 *   - Throttle управляется отдельно (клавиатура/геймпад), НЕ с телефона
 *
 * Установка:
 *   1. Повесить на тот же GameObject, что и Drone_Inputs
 *   2. На сцене должен быть GyroReceiver (на любом объекте)
 *   3. Включить "Use Head Tracking" в инспекторе
 *
 * Можно переключаться между обычным управлением и head tracking в рантайме.
 */
